...
get_bus_id() {
  local monitor_serial="$1"
  local detect_output="$2"
  echo "$detect_output" | grep -B 10 "$monitor_serial" | grep "I2C bus:" | grep -oP 'i2c-\K[0-9]+'
}
...
monitor_1_serial='1APR9UA001815'
...
monitor_2_serial='2OMR6UA000303'
...
monitor_3_serial='1APR9UA001811'
monitor_3_displayport_1=x0f
monitor_3_hdmi_1=x11
monitor_3_hdmi_2=x12
monitor_3_desktop_input=$monitor_3_hdmi_2
monitor_3_laptop_input=$monitor_3_displayport_1
start_time=$(date +%s.%N)
...
monitor_1_bus=$(get_bus_id "$monitor_1_serial" "$detect_output")
monitor_2_bus=$(get_bus_id "$monitor_2_serial" "$detect_output")
monitor_3_bus=$(get_bus_id "$monitor_3_serial" "$detect_output")
...
echo "monitor_3_bus: $monitor_3_bus"

# Toggle inputs for all monitors
if [ "$device" = "desktop" ]; then
  set_monitor_input "$monitor_1_bus" "$monitor_1_desktop_input" &
  set_monitor_input "$monitor_2_bus" "$monitor_2_desktop_input" &
  set_monitor_input "$monitor_3_bus" "$monitor_3_desktop_input" &
  ...
  set_monitor_input "$monitor_3_bus" "$monitor_3_laptop_input" &
fi
